...
export default function SideBar(props) {
    ...
    return (
        <>
            <div className={(props.hide ? " -translate-x-full " : "") + "backdrop-blur-sm absolute transform duration-300 select-none z-40 left-0 top-0 h-full pt-7 w-auto flex flex-col justify-start items-center border-black border-opacity-60 bg-black bg-opacity-50"}>
                {
                    (
...
export function AllApps(props) {
    ...
    return (
        ...
        >
             <img width="28px" height="28px" className="-7" src="./themes/Yaru/system/view-app-grid-symbolic.svg" alt="Ubuntu view app" />
            <div className="relative">
                <div
                    className={
                        (title ? " visible " : " invisible ") +
                        "font-sans w-max py-0.5 px-1.5 absolute top-1 left-full ml-5 text-ubt-grey text-opacity-90 text-sm bg-ub-grey bg-opacity-70 border-gray-400 border border-opacity-40 rounded-md"
                    }
                >
                    Show Applications
                </div>
